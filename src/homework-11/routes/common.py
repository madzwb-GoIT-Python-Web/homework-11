...
async def reads(skip: int = 0, limit: int = 100, session: Session = Depends(db), current_user: User = Depends(auth.get_current_user)):
    return await repository.reads(skip, limit, session)


@router.get("/{pid}", response_model=Type)
async def read(pid: int, session: Session = Depends(db), current_user: User = Depends(auth.get_current_user)):
...
async def create(model: Type, session: Session = Depends(db), current_user: User = Depends(auth.get_current_user)):
    return await repository.create(model, session)


@router.put("/{pid}", response_model=Type)
async def update(datas: Type, pid: int, session: Session = Depends(db), current_user: User = Depends(auth.get_current_user)):
...
async def delete(pid: int, session: Session = Depends(db), current_user: User = Depends(auth.get_current_user)):
